---
name: oracle-advisor
description: Use Oracle for a consultation requiring explicit transport or evidence of the selected model and reasoning effort.
---
...
This skill reuses the installed Oracle CLI and available MCP tools. Use only capabilities actually exposed by the current environment.
...
Use verified model and effort values from installed help and the user's request. These examples contain placeholders, not recommended current models:

```bash
oracle --engine api --model '<requested-model>' --wait \
  --prompt "Review this package metadata for compatibility risks." --file package.json

oracle --engine browser --model '<requested-model>' --browser-thinking-time '<requested-effort>' \
  --prompt "Review this package metadata for compatibility risks." --file package.json
...
Before browser execution, confirm the route uses a permitted tool-managed attachment. Never open raw CDP or WebSocket connections to the user's Chrome or extract cookies to bypass an unavailable route. Confirm the selected files and destination are authorized before sending; authentication alone is not permission to disclose private context.
